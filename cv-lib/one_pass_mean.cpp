...
One_pass_mean::One_pass_mean(Image *m_image, int m_window_rows, int m_window_cols) :
    image(m_image), window_rows(m_window_rows), window_cols(m_window_cols), image_rows(image->get_nrows()),
    image_cols(image->get_ncols()), n_image_pixels(image->get_npixels()), n_window_pixels(window_rows * window_cols),
    row_delta(std::round((window_rows - 1) / 2.0)), col_delta(std::round((window_cols - 1) / 2.0)) {
    init();
}
...
double One_pass_mean::get_mean() { return window_sum / n_window_pixels; }
