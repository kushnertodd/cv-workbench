...
int Hough_accum::get_nrhos() { return polar_trig->get_nrhos(); }
    ...
    for (int col = 0; col < get_ncols(); col++) {
        for (int row = 0; row < get_nrows(); row++) {
            ...
            if (value > image_theshold) {
                for (int theta_index = 0; theta_index < get_nthetas(); theta_index++) {
                    int rho_index = polar_trig->pixel_theta_to_rho(col, row, theta_index);
...
int point_theta_to_rho(int col, int row, int theta_index) {
    polar_trig->pixel_theta_to_rho(col, row, theta_index);
    ...
    int Hough_accum::rho_theta_to_index(int rho_index, int theta_index) {
        return rho_index * get_nthetas() + theta_index;
    }
