        ...
        if (!errors.has_error()) {
            ...
            if (!errors.has_error()) {
                if (operator_str == "erode") {
                    ...
                    if (!errors.has_error())
                        output_data_stores.front()->write_operator_image(
                                erode_image.get(), "Operator_filter_image_morphology::run", errors);
                    if (!errors.has_error())
                    ...
                    if (!errors.has_error())
                        output_data_stores.front()->write_operator_image(
                                dilate_image.get(), "Operator_filter_image_morphology::run", errors);
                    if (!errors.has_error())
                        dilate_image->log(log_entries);
                } else if (operator_str == "open") {
                    std::unique_ptr<Image> open_image(Morphology::open(input_image.get(), structuring_element_type,
                                                                       height, width, thickness, errors));
                    if (!errors.has_error())
                        output_data_stores.front()->write_operator_image(
                                open_image.get(), "Operator_filter_image_morphology::run", errors);
                    if (!errors.has_error())
                    ...
                    if (!errors.has_error())
                        output_data_stores.front()->write_operator_image(
                                close_image.get(), "Operator_filter_image_morphology::run", errors);
                    if (!errors.has_error())
                    ...
                    if (!errors.has_error())
                        output_data_stores.front()->write_operator_image(
                                gradient_image.get(), "Operator_filter_image_morphology::run", errors);
                    if (!errors.has_error())
                    ...
                    if (!errors.has_error())
                        output_data_stores.front()->write_operator_image(
                                top_hat_image.get(), "Operator_filter_image_morphology::run", errors);
                    if (!errors.has_error())
                    ...
                    if (!errors.has_error())
                        output_data_stores.front()->write_operator_image(
                                black_hat_image.get(), "Operator_filter_image_morphology::run", errors);
                    if (!errors.has_error())
