...
void Operator_filter_image_morphology::run(std::list<Data_source_descriptor *> &input_data_sources,
    ...
    if (!errors.has_error()) {
        ...
        Image *input = input_data_source->read_operator_image("Operator_filter_image_morphology::run", errors);
        if (!errors.has_error() && input != nullptr)
            input->check_grayscale("Operator_filter_image_morphology::run", errors);
        if (!errors.has_error() && input != nullptr) {
            if (operator_str == "erode") {
                std::unique_ptr<Image> erode_image(
                        Morphology::erode(input, structuring_element_type, height, width, thickness, errors));
                if (!errors.has_error())
                ...
                std::unique_ptr<Image> dilate_image(
                        Morphology::dilate(input, structuring_element_type, height, width, thickness, errors));
                if (!errors.has_error())
                ...
                std::unique_ptr<Image> open_image(
                        Morphology::open(input, structuring_element_type, height, width, thickness, errors));
                if (!errors.has_error())
                ...
                std::unique_ptr<Image> close_image(
                        Morphology::close(input, structuring_element_type, height, width, thickness, errors));
                if (!errors.has_error())
                ...
                std::unique_ptr<Image> gradient_image(
                        Morphology::gradient(input, structuring_element_type, height, width, thickness, errors));
                if (!errors.has_error())
                ...
                std::unique_ptr<Image> top_hat_image(
                        Morphology::top_hat(input, structuring_element_type, height, width, thickness, errors));
                if (!errors.has_error())
                ...
                std::unique_ptr<Image> black_hat_image(
                        Morphology::black_hat(input, structuring_element_type, height, width, thickness, errors));
                if (!errors.has_error())
