#include <iostream>
#include <memory>
#include "errors.hpp"
#include "image.hpp"
#include "wb_filename.hpp"
#include "wb_utils.hpp"

bool debug = false;

int main(int argc, char **argv) {
  if (argc != 3 && argc != 7)
    wb_utils::error_exit("usage: image-copy in_filename out-filename [min-row min_col max_row max_col]");
  bool have_subimage = (argc == 7);
  std::string in_filename = argv[1];
  std::string out_filename = argv[2];
  int min_row = 0;
  int min_col = 0;
  int max_row = 0;
  int max_col = 0;
  if (have_subimage) {
    if (!wb_utils::string_to_int(argv[3], min_row))
      wb_utils::error_exit("invalid min-row");
    if (!wb_utils::string_to_int(argv[4], min_col))
      wb_utils::error_exit("invalid min-col");
    if (!wb_utils::string_to_int(argv[5], max_row))
      wb_utils::error_exit("invalid max-row");
    if (!wb_utils::string_to_int(argv[6], max_col))
      wb_utils::error_exit("invalid max-col");
  }
  Errors errors;
  std::unique_ptr<Wb_filename> wb_in_filename(Wb_filename::create_wb_filename(in_filename, errors));
  errors.check_exit("invalid in-filename");
  std::unique_ptr<Wb_filename> wb_out_filename(Wb_filename::create_wb_filename(out_filename, errors));
  errors.check_exit("invalid out-filename");
  Image *in_image = nullptr;
  if (wb_in_filename->is_jpeg()) {
    in_image = Image::read_jpeg(in_filename, errors);
  } else if (wb_in_filename->is_bin()) {
    in_image = Image::read(in_filename, errors);
  } else {
    errors.add("", "", "invalid in-filename");
  }
  errors.check_exit("invalid filename");
  if (in_image != nullptr) {
    if (have_subimage)
      in_image->set_subimage(min_row, min_col, max_row, max_col, errors);
    if (!errors.has_error()) {
      if (wb_out_filename->is_jpeg()) {
        in_image->write_jpeg(out_filename, errors);
      } else if (wb_out_filename->is_bin()) {
        in_image->write(out_filename, errors);
      } else if (wb_out_filename->is_text()) {
        in_image->write_text(out_filename, "\t", errors);
      } else {
        errors.add("", "", "invalid out-filename");
      }
      errors.check_exit(out_filename + " image write failed");
    }
    errors.check_exit("invalid sub-image");
  }
}
