...
int main(int argc, char **argv) {
  ...
  if (wb_in_image_filename->is_jpeg()) {
    in_image = Image::read_jpeg(in_image_filename, errors);
  } else if (wb_in_image_filename->is_bin()) {
    in_image = Image::read(in_image_filename, errors);
  } else {
  ...
  errors.check_exit("invalid in-pattern");
  if (in_image != nullptr && in_pattern != nullptr) {
    Pearsons_correlation pearsons_correlation(in_image, in_pattern);
    Image *out_image = pearsons_correlation.correlate();
    std::string out_image_bin_filename = wb_out_image_filename->to_bin();
    out_image->write(out_image_bin_filename, errors);
    ...
    out_image->write_text(out_image_text_filename, "\t", errors);
