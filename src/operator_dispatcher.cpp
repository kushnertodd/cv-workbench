...
Operator *Operator_dispatcher::create_operator(const std::string &operator_name) {
  ...
  else if (operator_name == "operator-filter-edge-gaussian")    pOperator = new Operator_filter_edge_gaussian();
  else if (operator_name == "operator-filter-edge-laplacian")    pOperator = new Operator_filter_edge_laplacian();
  else if (operator_name == "operator-filter-smooth-average")    pOperator = new Operator_filter_smooth_average();
  else if (operator_name == "operator-filter-smooth-gaussian")    pOperator = new Operator_filter_smooth_gaussian();
  else if (operator_name == "operator-filter-smooth-median")    pOperator = new Operator_filter_smooth_median();
  else if (operator_name == "operator-histogram-hough-create")    pOperator = new Operator_histogram_hough_create();
  else if (operator_name == "operator-histogram-peak-detect")    pOperator = new Operator_histogram_peak_detect();
  else if (operator_name == "operator-hough-peak-detect")    pOperator = new Operator_hough_peak_detect();
  else if (operator_name == "operator-transform-image-copy")    pOperator = new Operator_transform_image_copy();
  else if (operator_name == "operator-transform-image-morphology")    pOperator = new Operator_transform_image_morphology();
  else
    pOperator = nullptr;
  return pOperator;
}