...
void Operator_filter_edge_prewitt::run(std::list<Data_source_descriptor *> &input_data_sources,
  ...
  bool orientation_found = Operator_utils::get_string_parameter("Operator_filter_edge_prewitt::run",
                                                                operator_parameters,
                                                                "orientation",
  ...
  if (!errors.has_error()) {
    ...
    if (!errors.has_error()) {
      ...
      if (!errors.has_error()) {
        std::unique_ptr<Image> output2(prewitt_kernel_col->convolve_numeric(output1.get(), errors));
