...
bool WB_window::clip_window(int rows, int cols, Line_segment &line_segment, Polar_line &line, int nrhos)  {
  ...
  if (line.theta_index <= theta_lower || line.theta_index > theta_upper) {
    ...
    // must clip against the window top and bottom first
    int col_at_window_top = Polar_trig::rho_theta_row_to_col(line.rho_index, line.theta_index, window_top, rows, cols, nrhos);
    int col_at_window_bottom = Polar_trig::rho_theta_row_to_col(line.rho_index, line.theta_index, window_bottom, rows, cols, nrhos);
    ...
    if (col_at_window_top < window_left
      ...
      int row_at_window_left = Polar_trig::rho_theta_col_to_row(line.rho_index, line.theta_index, window_left, rows, cols, nrhos);
      if (debug)
      ...
      int row_at_window_right = Polar_trig::rho_theta_col_to_row(line.rho_index, line.theta_index, window_right, rows, cols, nrhos);
      if (debug)
