...
Kernel::Kernel(int m_kernel_rows, int m_kernel_cols, CV_image_depth::Image_depth m_depth) :
...
Image *Kernel::convolve(Image *src) const {
  ...
  CV_image_depth::Image_depth src_depth = src->get_depth();
  int src_components = src->get_components();

  // output image is CV_image_depth::Image_depth::CV_32F if either the image and kernel are CV_image_depth::Image_depth::CV_32F, else it is CV_image_depth::Image_depth::CV_32S
  CV_image_depth::Image_depth out_depth =
      (src_depth == CV_image_depth::Image_depth::CV_32F || this->depth == CV_image_depth::Image_depth::CV_32F ? CV_image_depth::Image_depth::CV_32F : CV_image_depth::Image_depth::CV_32S);
...
Kernel *Kernel::create_32S(int kernel_rows, int kernel_cols, pixel_32S *buf_32S) {
  auto *kernel = new Kernel(kernel_rows, kernel_cols, CV_image_depth::Image_depth::CV_32S);
  kernel->add_32S(buf_32S, kernel->size);
  return kernel;
}

Kernel *Kernel::create_32F(int kernel_rows, int kernel_cols, pixel_32F *buf_32F) {
  auto *kernel = new Kernel(kernel_rows, kernel_cols, CV_image_depth::Image_depth::CV_32F);
...
std::string Kernel::to_string() const {
  ...
  os << "kernel_rows " << kernel_rows
     ...
     << " depth "  << " " << CV_image_depth::to_string(depth)
     << " size " << size;
  return os.str();
}